#' Main function of GMStools
#'
#' Marker_selection function will select markers for genomic prediction with best accuracy.
#' @param ini_snps_bk initial SNP backup value
#' @param init_selsnp the number of selected SNPs
#' @param j the number of cross validation
#' @param cv_samples the given vector of cross-validation sample vectors
#' @param mm the given method for genomic prediction
#' @param geno2 filtered genotype dataset
#' @param phenotype1 filtered phenotype dataset
#' @param preset_fname the given preset SNPs which have to included
#' @param ix a vector which is ordered SNPs by p-values in GWAS
#' @param acc1 the given accuracy to goal of prediction
#' @param allm If it is TRUE, calculate prediction accuracy for all input markers
#' @param cv the given cross-validation number
#' @param time_cutoff the given time (hour) for preventing to longer computation time
#' @param delta the given value that is a cut-off of increasing accuracy rate
#' @importFrom caret findCorrelation
#' @importFrom stats cor
#' @importFrom utils write.table
#' @return Results files will be generated for each CV steps
#' @export
GMS_main <- function(ini_snps_bk, init_selsnp, j, mm, cv_samples, geno2,
                     phenotype1, preset_fname, ix, acc1, allm, cv, time_cutoff, delta){
